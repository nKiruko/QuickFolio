...
import { InquiryData } from "../pages/inquiryPortal";



const Inquiry: FunctionComponent<{testData: any}> = ({ testData }) => {
    return(
        <div className="relative md:w-5/6">
        <input type="checkbox" className="peer absolute top-0 inset-x-0 w-full h-12 opacity-0 z-10 cursor-pointer"/>
                <div className="bg-dinoblack h-12 w-full pl-5 flex items-center">
                    <h1 className="text-lg font-heading text-white float-left px-2">Sender&#58; {testData.firstname}</h1>
        ...
        <div className="bg-dinogrey bg-opacity-50 overflow-hidden max-h-0 peer-checked:max-h-full">
            ...
            <p className="pb-5">{testData.message}</p>
